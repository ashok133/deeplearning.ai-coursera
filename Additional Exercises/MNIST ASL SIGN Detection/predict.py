...
def predict_sign(image_loc):
    image_rgb = plt.imread(image_loc, 0)
    image = rgb2gray(image_rgb)
    # image = Image.open(image_loc).convert('LA')
    print(image.shape)
    print("image.shape**********")
    with open('learned_model/params.pickle', 'rb') as handle:
        params = pickle.load(handle)
        my_image_resized = scipy.misc.imresize(image, size=(28,28))
        print(my_image_resized.shape)
        my_image = my_image_resized.reshape((1, 28*28)).T
...
predict_sign(sys.argv[1])

# img_to_base64(sys.argv[0])
